...
class ClipMatcher(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        b, t = segment.shape[:2]
        if use_contrastive:
            segment = rearrange(rt_pos_queries, 'b t c h w -> (b t) c h w')
        segment = rearrange(segment, 'b t c h w -> (b t) c h w')
        device = segment.device
        
        # lt_track query change
        if lt_track_query is not None:
            if random.randint(0, 1) == 1:
                query = lt_track_query
                ...
                if rt_pos:
                    ...
                    if random.randint(0, 1) == 1:
                        ...
                        if batch_has_valid.any():
                            if thr_hard:
                                valid_sim = sim.masked_fill(~sim_mask, float('inf'))
                                rand_indices_per_batch[batch_has_valid] = valid_sim[batch_has_valid].argmin(dim=-1) # [b]
                            else:
                        ...
                        if enable_rt_pq_threshold:
                            ...
                            if query_jitter:
                                # Direct tensor operations to optimize the logic
                                mask_zero = top_sim_idx == 0
                                mask_last = top_sim_idx == (t - 1)

                                # If top_sim_idx is 0, add 1
                                top_sim_idx = top_sim_idx + mask_zero.long()

                                # If top_sim_idx is t - 1, subtract 1
                                top_sim_idx = top_sim_idx - mask_last.long()

                                # Otherwise, randomly add or subtract 1
                                mask_other = ~(mask_zero | mask_last)
                                if top_sim_idx[mask_other].numel() > 0:  # Check if there are elements to modify
                                    random_changes = torch.tensor([random.choice([-1, 1]) for _ in range(top_sim_idx[mask_other].numel())]).reshape(-1)
                                    random_changes = random_changes.to(device)
                                    
                                    top_sim_idx[mask_other] += random_changes
                                query = rt_pos_queries[torch.arange(b), top_sim_idx]
                            else:
                                query = rt_pos_queries[torch.arange(b), top_sim_idx] # [b,c,h2,w2]
                                
                        del rt_pos_queries_feat, query_feat, sim, sim_mask, batch_has_valid, rand_indices_per_batch, top_sim_idx, final_top_sim_idx
        ...
        #     output_dict = {}
        #     MIN_PROB = 1e-7
        #     clip_feat = rearrange(clip_feat, '(b t) c h w -> b t (c h w)', b=b, t=t)  # [b,t,c * h * w]
        #     sim = F.cosine_similarity(clip_feat, clip_feat, dim = 2) # [b,t]
        #     probs = F.softmax(sim, dim=-1)  # [b,t]
        #     probs_pos = (probs * gt_mats).sum(dim=-1)  # [b]
        #     contrastive_loss = -torch.log(probs_pos.clamp(min=MIN_PROB, max=1))  # [b]
        #     total_loss = {'contrastive_loss': contrastive_loss}
        ...
        # def multiple_positive_loss(sim, gt_mat: torch.Tensor):
            # sim: [N_q, N_cap]
            # gt_mat: [N_q, N_cap], binary -> from dataset.py
        #     MIN_PROB = 1e-7
