...
class ClipMatcher(nn.Module):
    ...
    def extract_feature(self, x, return_h_w=False, token_prune=False) -> torch.Tensor | tuple[torch.Tensor, int, int]:
    ...
    def token_prune_merge_advanced_plus_mat(self, images, if_adaptive=True, reduction_ratio=1/8):
        ...
        # # # Define local variables to capture hook outputs
        # desired_k = []
        # desired_q = []

        # # Hook function to capture k
        # def hook_k(module, input, output):
        #     desired_k.append(output)

        # # Hook function to capture q
        # def hook_q(module, input, output):
        #     desired_q.append(output)
        
        # # Set hooks for extracting the desired layer's k and q
        # hook_handle_k = self.backbone.encoder.layer[-1].attention.attention.query.register_forward_hook(hook_k)
        # hook_handle_q = self.backbone.encoder.layer[-1].attention.attention.query.register_forward_hook(hook_q)

        # Forward pass
        image_forward_outs = self.backbone.forward(images, output_hidden_states=True)
        image_features = image_forward_outs.hidden_states[self.select_layer]  # Exclude CLS token


        # Extract desired layer's k and q and remove hooks; calculate attention
        # try:
        #     desired_layer_k = desired_k[0]
        #     desired_layer_q = desired_q[0]
        # except:
        #     print(print(f'image_features {image_features.shape}, desired_layer {len(desired_k)}, {len(desired_q)}'))
        # hook_handle_k.remove()
        # hook_handle_q.remove()
        
        # Manually extract desired layer's k and q (attention weights) from the last layer
        last_layer = self.backbone.encoder.layer[-1]
        desired_layer_q = last_layer.attention.attention.query(image_features)   # Query
        desired_layer_k = last_layer.attention.attention.key(image_features)    # Key
        image_features = image_features[:, 1:]
        B, N, C = image_features.shape
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            with torch.autocast(device_type="cuda", dtype=self.backbone_dtype, enabled=self.backbone_autocast):
                query_feat, h, w = self.extract_feature(query, return_h_w=True)    # [b,c,h,w]
                clip_feat, idx = self.extract_feature(segment, token_prune=True)   # [b*t,c,left_tokens], [b*t,left_tokens]
        ...
        query_feat = rearrange(query_feat.unsqueeze(1).repeat(1,t,1,1), 'b t c n -> (b t) n c')# [b*t,n,c]
        clip_feat = rearrange(clip_feat, 'b c left_tokens -> b left_tokens c')            # [b*t,left_tokens,c]
        for layer in self.CQ_corr_transformer:
            clip_feat = layer(clip_feat, query_feat)                        # [b*t,left_tokens,c]
        clip_feat = rearrange(clip_feat, 'b left_tokens c -> b c left_tokens')  # [b*t,c,left_tokens]
