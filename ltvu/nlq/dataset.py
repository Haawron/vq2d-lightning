...
class NLQDataset(BaseDataset):
    def __init__(self, data_dir, split, feature_type, max_v_len, p_env_feat_dir):
        super().__init__(data_dir, split, feature_type, max_v_len)
        if enable_env_feature:
            self.p_env_feat_dir = Path(p_env_feat_dir)
...
class JointDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_dataset = JointDataset([
                ...
                NLQDataset('data/unified', train_split, self.config.feature_type, self.config.max_v_len, self.config.p_env_feat_dir)
        ...
        for split in self.config.test_splits:
            if split == 'QaEgo4D_test':
                ...
                test_datasets.append(NLQDataset('data/unified', split, self.config.feature_type, self.config.max_v_len, self.config.p_env_feat_dir))
            else:
                print(split)
                raise NotImplementedError
