...
REPORTED_INVALID_CLIP_UIDS = {
    # vq2d test
    'b6061527-3ae2-46b4-b050-15eddc1967bb',
}
...
def main(
    ...
    Run preprocessing for training split:
        ...
        python -m ltvu.preprocess.extract_frames  # will process both train and val split

    Run preprocessing in parallel for 4 ranks:

        for rank in {0..3}; do
            python -m ltvu.preprocess.extract_frames --world_size 4 --rank $rank &
        done

    Run preprocessing for validation split for evaluation:

        python -m ltvu.preprocess.extract_frames --split val --whole
    """
