...
class VQ2DFitDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config: DictConfig, split: str = 'train'):
        ...
        self.contrastive = config.get('contrastive')
        
        if self.lt_track_query:
            self.p_lt_track = Path(self.lt_track_query.lt_track_query_dir)
            self.p_lt_track_ann = self.p_anns_dir / f'egotracks_v1_{split}.json'
            self.all_anns_lt = json.load(self.p_lt_track_ann.open())
            new_all_anns_lt = {}
            for all_ann_lt in self.all_anns_lt:
                key = f'{all_ann_lt["clip_uid"]}_{all_ann_lt["object_title"]}'
                new_all_anns_lt[key] = {
                    'clip_uid': all_ann_lt['clip_uid'],
                    'annotation_uid': all_ann_lt['annotation_uid'],
                    'lt_track': all_ann_lt['lt_track']
                    }
            self.all_anns_lt = new_all_anns_lt
        else:
            self.all_anns_lt = None
            self.p_lt_track = None
    ...
    def __getitem__(self, idx):
        ...
        if self.rt_pos_query and self.split == 'train':
            rt_pos_queries, gt_mats = self.get_rt_pos_query(ann, frame_idxs, segment, self.contrastive)
            
        segment, gt_rt = self.pad_and_resize(segment, gt_rt)  # [t, c, s, s], [t, 4]
        query = self.get_query(ann)
        
        if self.lt_track_query is not None and self.split == 'train':
            lt_track_query = self.get_lt_track_query(ann)
            
            if lt_track_query is None:
                lt_track_query = query
        ...
        if self.rt_pos_query and self.split == 'train':
            ...
            (sample
                .setdefault('experiment', {})
                .setdefault('multi_query', {})
                .setdefault('gt_mats', gt_mats))
            
        if self.lt_track_query is not None and self.split == 'train':
            (sample
                .setdefault('experiment', {})
                .setdefault('multi_query', {})
                .setdefault('lt_track_query', lt_track_query))
        return sample
    ...
    def get_lt_track_query(self, ann):                
        p_lt_track_frame = self.p_lt_track / ann['clip_uid'] / ann['object_title']
        if p_lt_track_frame.exists():
            frames = p_lt_track_frame.glob('*.jpg')
            frame = random.choice(list(frames))
            fno = int(frame.stem.split('_')[-1])
            p_frame = p_lt_track_frame / frame
            
            ann_lt = self.all_anns_lt[f'{ann["clip_uid"]}_{ann["object_title"]}']
            oh, ow = ann_lt['original_height'], ann_lt['original_width']
            
            idx = fno  - ann_lt['lt_track'][0]['fno']
            x, y, w, h = ann_lt['lt_track'][idx]['x'], ann_lt['lt_track'][idx]['y'], ann_lt['lt_track'][idx]['w'], ann_lt['lt_track'][idx]['h']
    ...
    def get_rt_pos_query(self, ann, frame_idxs, segment, contrastive=False): 
        def query_padding(frame, h,w):
            l, s = max(h, w), min(h, w)
            pad_size = (l - s) // 2
            if h > w:
        ...
        for idx, frame_idx in enumerate(frame_idxs):
            if frame_idx in list(rt_ann.keys()):
                ...
                if self.query_padding:
                    frame = query_padding(frame, rt_ann[frame_idx]['h'], rt_ann[frame_idx]['w'])
                ...
                if contrastive:
                    # random_crop
                    frame = segment[idx]
                    x1 = random.randint(0, frame.shape[1])
                    y1 = random.randint(0, frame.shape[2])
                    w = random.randint(0, frame.shape[1] - x1)
                    h = random.randint(0, frame.shape[2] - y1)
                    frame = frame[:, x1:x1+w, y1:y1+h]
                    
                    if self.query_padding:
                        frame = query_padding(frame, h, w)
                    frame = F.interpolate(frame[None], size=self.query_size, mode='bilinear', align_corners=True, antialias=True)
                else:
