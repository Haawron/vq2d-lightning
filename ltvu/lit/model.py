...
class LitModule(L.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.vq2d_egotracks = config.get('vq2d_egotracks')
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.rt_pos_query is not None:
            ...
            self.thr_curriculum = self.rt_pos_query.thr_curriculum
            self.sort_curriculum = self.rt_pos_query.sort_curriculum
            extra_args['sim_thr']=self.sim_thr
            extra_args['enable_rt_pq_threshold']=self.rt_pos_query.enable_rt_pq_threshold
            extra_args['query_jitter']=self.query_jitter
            
            if self.thr_curriculum:
                extra_args['rt_pos']=True
                extra_args['enable_rt_pq_threshold']=True
                if self.current_epoch >= self.config.trainer.max_epochs * (1/3):
                    extra_args['sim_thr']=self.sim_thr - 0.1
                elif self.current_epoch >= self.config.trainer.max_epochs * (2/3):
                    extra_args['sim_thr']=self.sim_thr - 0.2      
            elif self.sort_curriculum:
                extra_args['rt_pos']=True
                if self.current_epoch >= self.config.trainer.max_epochs / 2:
                    extra_args['enable_rt_pq_threshold'] = True
                    extra_args['thr_hard'] = True
                else:
        ...
        # if self.contrastive:
        #     output_dict_contrastive = self.model.forward_contrastive(**batch, compute_loss=True, use_contrastive=True, **extra_args)
            
        #     output_dict['loss'] = output_dict['loss'] + output_dict_contrastive['loss']
        #     output_dict['log_dict'].update(output_dict_contrastive['log_dict'])
